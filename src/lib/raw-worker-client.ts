 ...
 *
 * Deliberately does not touch `queue`. Frames already queued behind the dead
 * one are chained off *its* promise, not off this module's `queue` variable,
 * so resetting `queue` here would not reach them anyway -- and if a call site
 * changes that stops being true, resetting it would let two queued frames
 * both resolve on the same tick and both reach the fresh worker's
 * `postMessage` before either's response arrives, sharing one
 * `WasmToolRunner` and breaking the one-frame-at-a-time invariant this
