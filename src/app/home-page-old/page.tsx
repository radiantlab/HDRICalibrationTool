...
export default function Home() {
	...
	function allInputsEntered() {
		...
		if (!cf_correctionPaths)
			missingInputs.push("Calibration Factor Correction");
		if (!nd_correctionPaths) missingInputs.push("Neutral Density Correction");
		if (!viewSettings.diameter) missingInputs.push("Diameter");
		if (!viewSettings.xleft) missingInputs.push("X Left Offset");
		if (!viewSettings.ydown) missingInputs.push("Y Bottom Offset");
		...
		Object.entries(luminanceSettings).forEach(([key, value]) => {
			...
			if (!value && enteredSetting) {
				...
				name = key
					...
					.join(" ");
				missingInputs.push(`${name} in Falsecolor Settings`);
			}
		});
	...
	return (
		<div className="bg-gray-300 text-black grid grid-cols-4 min-h-screen">
			<main className="bg-white col-span-4 m-8 mt-0 p-5 border-l border-r border-gray-400 overflow-y-auto">
				{/* Progress Bar */}
