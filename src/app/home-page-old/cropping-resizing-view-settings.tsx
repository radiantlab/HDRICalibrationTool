...
export default function CroppingResizingViewSettings() {
	...
	async function dialog() {
		...
		let imgsLoc = "";
		if (
			valid_extensions.includes(Extensions(devicePaths[0]).toLocaleLowerCase())
		) {
		...
		if (selected !== null) {
			if (!valid_extensions.includes(Extensions(selected).toLowerCase())) {
				alert(
					"Invalid file type. Please only enter valid image types: jpg, jpeg, tif, tiff, or raw image formats."
				);
				return;
			}
			...
			if (ext !== "jpeg" && ext !== "jpg" && ext !== "tif" && ext !== "tiff") {
				const tiff: any = await invoke<string>("convert_raw_img", {
