...
function getNavigatorGPU() {
	...
	if (!maybeGPU) return null;
	if (typeof (maybeGPU as { requestAdapter?: unknown }).requestAdapter !== "function") {
		return null;
	}
	return maybeGPU as {
...
async function computeWebGpuLuminance(
	...
	const gpuMapMode = (globalThis as { GPUMapMode?: Record<string, number> })
		.GPUMapMode;
	if (!gpuBufferUsage || !gpuMapMode) return null;
  ...
  luminanceValues[pixelIndex] = params.scale * bright;
	...
	try {
		...
		const bindGroup = device.createBindGroup({
			layout: (pipeline as { getBindGroupLayout: (index: number) => unknown }).getBindGroupLayout(
				0
			),
			entries: [
