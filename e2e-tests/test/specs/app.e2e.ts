...
const responseFunctionPath = fileURLToPath(
	new URL("../inputs/response_function_files/Response_function.rsp", import.meta.url),
);
...
const neutralDensityCorrectionPath = fileURLToPath(
	new URL("../inputs/calibration_files/NDfilter_no_transform.cal", import.meta.url),
);
	...
	await browser.waitUntil(
		async () =>
			await browser.execute((inputSelector) => {
				return Boolean(document.querySelector(inputSelector));
			}, selector),
		{
			timeout: 10000,
...
function getPipelineFailureMessage(outputDir: string): string | null {
	...
	const newestTracePath = path.join(traceDir, traceFiles[traceFiles.length - 1]!);
...
describe("HDRI Calibration Tool", () => {
	...
	it("generates an HDR image", async () => {
		...
		await setTextInputValue('input[name="lensMask.x"]', lensInformation.x.toString());
		await setTextInputValue('input[name="lensMask.y"]', lensInformation.y.toString());
		...
		await browser.execute(() => {
			const button = Array.from(document.querySelectorAll("button")).find((element) =>
				element.textContent?.includes("Generate HDR Image"),
			);
		...
		await imageViewerInput.waitForDisplayed({ timeout: 10000 });
