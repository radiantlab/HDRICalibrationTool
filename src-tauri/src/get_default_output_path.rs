...
pub async fn get_default_output_path() -> Result<String, String> {
    if let Some(user_dirs) = UserDirs::new() {
        if let Some(documents_dir) = user_dirs.document_dir() {
            ...
            if !target_dir.exists() {
                ...
                if let Err(error) = fs::create_dir_all(&target_dir) {
                    return Err(format!(
                        "get_default_output_path: create_dir_all: {}",
                        error
                    ));
                }
            }
        ...
        return Err("get_default_output_path: could not find 'documents' directory".to_string());
        // documents_dir is None
    }
    return Err("get_default_output_path: could not find 'user' directory".to_string());
    // user_dirs is None
}
