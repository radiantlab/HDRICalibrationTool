 ...
 */
#[tauri::command]
pub fn display_hdr_img(radiance_path: String, image_path: String) -> Result<String, String> {
    ...
    if !stat_res.is_ok() || !stat_res.unwrap_or(ExitStatus::default()).success() {
        return Err(
            "Error, non-zero exit status. ximage command (display hdr image) failed.".to_string(),
        );
    }
